...
<div class="d-flex justify-content-center">
	<div class="w-50">
		...
		<c:if test="${not empty post.imagePath}">
			<div class="my-3">
				<img src="${post.imagePath}" alt="업로드 이미지" width="400">
			</div>
		</c:if>
		...
		<div class="d-flex justify-content-between">
			...
			<div>
				...
				<button type="button" id="updateBtn" class="btn btn-primary" data-post-id="${post.id}">수정</button>
			</div>
		</div>
	</div>
</div>
...
$(document).ready(function(){
	...
	$('#updateBtn').on('click', function(){
		...
		let file = $('#file').val(); // C:\fakepath\PurpleMoon.png
		...
		$.ajax({
			...
			, error: function(request, status, error){
				alert("메모 수정 실패했습니다. 관리자 문의 바랍니다.");
			}
		});
	});
});
